# ## HERE IS THE TRUE ABC PROCESS. 
# SEED = 111L


# source("functions4abc.R")

# args <- commandArgs(trailingOnly = TRUE)

# d <- as.numeric(args[1])
# t <- as.numeric(args[2])
# l <- as.numeric(args[3])
# rho <- as.numeric(args[4])

# print(paste(d,t,l,rho, sep=" - "))
# # start from an observed gene tree and an observed set of gene trees. 
# # WE DO A SELF-TEST, so to get these trees for now we do simulations with known parameters
# N_extant <- 25L #L is to force it to be an integer (not a double)
# target_rho <- rho # proportion of extant species that are sampled (i.e. that are tips of the observed species tree)
# #dtl <- sample_theta()
# dtl <- c(d=d,t=t,l=l)
# N_gene_trees <- 40L # Number of observed gene trees

# # SIMULATE SPECIES TREE
# birth <- 1.0
# death <- 0
# sp_tree <- simulate_species_tree(N_extant, birth, death) ## -> PASS BY RPHYLO MAYBE? 
# sp_tree_ape <- read.tree(text=tree_to_newick(sp_tree))       # ape format
# sp_tree_ape$root.edge <- 0 #no branch before root to ensure mrca(aug.tree) = mrca(tree)
# sp_tree_ape$tip.label <- paste0("e", sp_tree_ape$tip.label)
# #plot(sp_tree_ape)
# # We augment the tree to add ghosts (here for now only extant but unsampled species)
# augmented_sp_tree_ape <- augment_tree(sp_tree_ape, 4,0,target_rho, 1,1)
# augmented_sp_tree_rustree <- parse_newick(write.tree(augmented_sp_tree_ape))
# true_rho <- N_extant / Ntip(augmented_sp_tree_ape)
# observed_gene_trees <- sim_gene_trees_fun(augmented_sp_tree_rustree, N_gene_trees, dtl["d"], dtl["t"], dtl["l"], seed=SEED)
# obs_SCORES <- do.call(rbind, lapply(observed_gene_trees, compute_informative_scores))






# smc_abc_eval <- function(THETAS, sp_tree_ape, N_gene_trees, obs_SCORES) {
#   p <- 10 #10 scores
#   print("Augmenting trees...")  
#   multi_augmented_sp_trees_ape <- augment_tree_batch(
#     sp_tree_ape, 4, 0,
#     THETAS[, "rho"], 1, 1
#   )
#   print("Augmenting trees... DONE")
#   ks_matrix <- matrix(NA_real_, nrow(THETAS), p)
#   for (i in seq_len(nrow(THETAS))) {
#     theta <- THETAS[i, ]
#     if (i %% 50 == 0) {
#       gc()
#       cat(paste(i,paste(theta,collapse="-"), sep=" -> "), "\n")
#     }

#     sp_tree_aug <- parse_newick(write.tree(
#       multi_augmented_sp_trees_ape$trees[[i]]
#     ))
#     simulated_trees <- sim_gene_trees_fun(
#       sp_tree_aug,
#       N_gene_trees,
#       theta[1], theta[2], theta[3], 
#       seed = SEED
#     )
#     if (is.null(simulated_trees) || length(simulated_trees) < 4) {
#       ks_matrix[i, ] <- rep(1, p)  # worst possible distance
#       print("NOTE: Not enough gene trees with at least 4 tips, skipping this theta.")
#       next
#     }
#     scores <- t(vapply(simulated_trees,compute_informative_scores,numeric(p)))
#     ks_matrix[i, ] <- compute_ks_vector(obs_SCORES, scores)
#     rm(simulated_trees, scores)
#   }
#   list(
#     ks_matrix = ks_matrix,
#     d_abc = rowMeans(ks_matrix)
#   )
# }


# get_new_thetas <- function(THET, dabc, epsilon=0.05, N_keep=40, N_simulations=400) {
#   THETAS_sel <- THET[order(dabc)[1:N_keep],] #approximate posterior sample
#   THETAS_new <- NULL
#   for (i in seq_len(N_simulations)) {
#     base <- THETAS_sel[sample(nrow(THETAS_sel), 1), ] # sample one retained particle
#     noise <- runif(length(base), -epsilon, epsilon) # add random perturbation in [-epsilon, epsilon]
#     theta <- base + noise
#     theta[theta < 0] <- -theta[theta < 0] #reflective boundary at 0
#     theta[theta > 1] <- 2 - theta[theta > 1] #reflective boundary at 1
#     #theta <- pmin(pmax(theta, 0), 1) # prevent negative values and values > 1 
#     THETAS_new <- rbind(THETAS_new, theta)
#   }
#   THETAS_new
# }

# N_simulations <- 400
# # ABC STEP 1
# THETAS_init <- sample_theta(N_simulations) # Sample DTL values
# THETAS_init$rho <- runif(N_simulations, 0, 1) # Sample RHO values#
# res1 <- smc_abc_eval(THETAS_init, sp_tree_ape, N_gene_trees, obs_SCORES)
# # ABC STEP 2
# THETAS1 <- get_new_thetas(THETAS_init, res1$d_abc, epsilon=0.15)
# {
#   res2 <- smc_abc_eval(THETAS1, sp_tree_ape, N_gene_trees, obs_SCORES)
# # ABC STEP 3
# THETAS2 <- get_new_thetas(THETAS1, res2$d_abc, epsilon=0.12)
# res3 <- smc_abc_eval(THETAS2, sp_tree_ape, N_gene_trees, obs_SCORES)
# # ABC STEP 4
# THETAS3 <- get_new_thetas(THETAS2, res3$d_abc, epsilon=0.10)
# res4 <- smc_abc_eval(THETAS3, sp_tree_ape, N_gene_trees, obs_SCORES)
# # ABC STEP 5
# THETAS4 <- get_new_thetas(THETAS3, res4$d_abc, epsilon=0.08)
# res5 <- smc_abc_eval(THETAS4, sp_tree_ape, N_gene_trees, obs_SCORES)
# # ABC STEP 6
# THETAS5 <- get_new_thetas(THETAS4, res5$d_abc, epsilon=0.06)
# res6 <- smc_abc_eval(THETAS5, sp_tree_ape, N_gene_trees, obs_SCORES)
# # ABC STEP 7
# THETAS6 <- get_new_thetas(THETAS5, res6$d_abc, epsilon=0.05)
# res7 <- smc_abc_eval(THETAS6, sp_tree_ape, N_gene_trees, obs_SCORES)

# THETAS7 <- get_new_thetas(THETAS6, res7$d_abc, epsilon=0.05)
# }




# ######################
# ## PLOT             ##
# ######################

# theta_list <- list(
#   THETAS_init,
#   THETAS1,
#   THETAS2,
#   THETAS3,
#   THETAS4,
#   THETAS5,
#   THETAS6, 
#   THETAS7
# )

# d_list <- list(
#   res1$d_abc,
#   res2$d_abc,
#   res3$d_abc,
#   res4$d_abc,
#   res5$d_abc,
#   res6$d_abc,
#   res7$d_abc
# )

# pdf(sprintf(
#   "results7/res_d%.3f_t%.3f_l%.3f_rho%.3f.pdf",
#   d,t,l,rho
# ), width=8, height=8)


# par(mfrow = c(2,2))

# params <- c("d","t","l","rho")
# true_vals <- c(dtl[1], dtl[2], dtl[3], true_rho)

# for (j in 1:4) {

#   boxplot(
#     lapply(theta_list, function(x) x[, j]),
#     main = params[j],
#     xlab = "Iteration",
#     ylab = "Value"
#   )

#   abline(h = true_vals[j], col = "red", lty = 2, lwd = 2)
# }

# dev.off()

# save(theta_list,d_list, dtl,true_rho,
#   file = sprintf(
#     "results7/res_d%.3f_t%.3f_l%.3f_rho%.3f.Rdata",
#     d,t,l,rho
#   )
# )


# # aug_low  <- augment_tree(sp_tree_ape, 4, 0, 0.01, 1, 1)
# # aug_high <- augment_tree(sp_tree_ape, 4, 0, 1, 1, 1)

# # gt_low  <- sim_gene_trees_fun(parse_newick(write.tree(aug_low)),  100, 0.05, 0.0, 0.05, 114L)
# # gt_high <- sim_gene_trees_fun(parse_newick(write.tree(aug_high)), 100, 0.05, 0.0, 0.05, 114L)

# # # Check how many tips survive on average
# # mean(sapply(gt_low,  Ntip))
# # mean(sapply(gt_high, Ntip))

# # obs_SCORES_low <- do.call(rbind, lapply(gt_low, compute_informative_scores))
# # obs_SCORES_high <- do.call(rbind, lapply(gt_high, compute_informative_scores))

# # score_names <- colnames(obs_SCORES_low)  # or paste0("S", 1:11) if no colnames

# # comparison <- data.frame(
# #   score    = score_names,
# #   mean_low  = colMeans(obs_SCORES_low),
# #   mean_high = colMeans(obs_SCORES_high),
# #   diff      = colMeans(obs_SCORES_low) - colMeans(obs_SCORES_high),
# #   ks_pval   = sapply(seq_len(ncol(obs_SCORES_low)), function(j)
# #                 ks.test(obs_SCORES_low[, j], obs_SCORES_high[, j])$p.value)
# # )

# # print(comparison, digits = 3)

SEED = 111L

source("functions4abc.R")

args <- commandArgs(trailingOnly = TRUE)

d <- as.numeric(args[1])
t <- as.numeric(args[2])
l <- as.numeric(args[3])
rho <- as.numeric(args[4])

print(paste(d,t,l,rho, sep=" - "))

N_extant <- 25L
target_rho <- rho
dtl <- c(d=d,t=t,l=l)
N_gene_trees <- 40L

# SIMULATE SPECIES TREE
birth <- 1.0
death <- 0
sp_tree <- simulate_species_tree(N_extant, birth, death)
sp_tree_ape <- read.tree(text=tree_to_newick(sp_tree))
sp_tree_ape$root.edge <- 0
sp_tree_ape$tip.label <- paste0("e", sp_tree_ape$tip.label)

# AUGMENT AND SIMULATE OBSERVED GENE TREES
augmented_sp_tree_ape <- augment_tree(sp_tree_ape, 4, 0, target_rho, 1, 1)
augmented_sp_tree_rustree <- parse_newick(write.tree(augmented_sp_tree_ape))
true_rho <- N_extant / Ntip(augmented_sp_tree_ape)
observed_gene_trees <- sim_gene_trees_fun(augmented_sp_tree_rustree, N_gene_trees, dtl["d"], dtl["t"], dtl["l"], seed=SEED)
obs_SCORES <- do.call(rbind, lapply(observed_gene_trees, compute_informative_scores))


smc_abc_eval <- function(THETAS, sp_tree_ape, N_gene_trees, obs_SCORES) {
  p <- 11
  print("Augmenting trees...")
  multi_augmented_sp_trees_ape <- augment_tree_batch(
    sp_tree_ape, 4, 0,
    THETAS[, "rho"], 1, 1
  )
  print("Augmenting trees... DONE")
  ks_matrix <- matrix(NA_real_, nrow(THETAS), p)
  for (i in seq_len(nrow(THETAS))) {
    theta <- THETAS[i, ]
    if (i %% 50 == 0) {
      gc()
      cat(paste(i, paste(theta, collapse="-"), sep=" -> "), "\n")
    }
    sp_tree_aug <- parse_newick(write.tree(
      multi_augmented_sp_trees_ape$trees[[i]]
    ))
    simulated_trees <- sim_gene_trees_fun(
      sp_tree_aug,
      N_gene_trees,
      theta[1], theta[2], theta[3],
      seed = SEED
    )
    if (is.null(simulated_trees) || length(simulated_trees) < 4) {
      ks_matrix[i, ] <- rep(1, p)
      next
    }
    scores <- t(vapply(simulated_trees, compute_informative_scores, numeric(p)))
    ks_matrix[i, ] <- compute_ks_vector(obs_SCORES, scores)
    rm(simulated_trees, scores)
  }
  list(
    ks_matrix = ks_matrix,
    d_abc = rowMeans(ks_matrix)
  )
}


get_new_thetas <- function(THET, dabc, epsilon=0.05, N_keep=40, N_simulations=400) {
  THETAS_sel <- THET[order(dabc)[1:N_keep], ]
  THETAS_new <- NULL
  for (i in seq_len(N_simulations)) {
    base <- THETAS_sel[sample(nrow(THETAS_sel), 1), ]
    noise <- runif(length(base), -epsilon, epsilon)
    theta <- base + noise
    theta[theta < 0] <- -theta[theta < 0]
    theta[theta > 1] <- 2 - theta[theta > 1]
    THETAS_new <- rbind(THETAS_new, theta)
  }
  THETAS_new
}


######################
## SMC-ABC LOOP     ##
######################

N_simulations <- 400
epsilon_schedule <- c(0.15, 0.12, 0.10, 0.08, 0.06, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05) # schedule of epsilon values for each iteration
min_rounds <- 6
max_rounds <- length(epsilon_schedule)
tol_improvement <- 0.0001

# STEP 1
THETAS_init <- sample_theta(N_simulations)
THETAS_init$rho <- runif(N_simulations, 0, 1)
res1 <- smc_abc_eval(THETAS_init, sp_tree_ape, N_gene_trees, obs_SCORES)
cat(sprintf("Round 1: median d_abc = %.4f\n", median(res1$d_abc)))

theta_list <- list(THETAS_init)
d_list <- list(res1$d_abc)

THETAS_current <- THETAS_init
res_current <- res1
round <- 1

while (round < max_rounds) {
  round <- round + 1
  epsilon <- epsilon_schedule[round]

  THETAS_new <- get_new_thetas(THETAS_current, res_current$d_abc, epsilon = epsilon)
  res_new <- smc_abc_eval(THETAS_new, sp_tree_ape, N_gene_trees, obs_SCORES)

  theta_list <- c(theta_list, list(THETAS_new))
  d_list <- c(d_list, list(res_new$d_abc))

  improvement <- median(res_current$d_abc) - median(res_new$d_abc)
  cat(sprintf("Round %d (epsilon=%.2f): median d_abc = %.4f, improvement = %.4f\n",
              round, epsilon, median(res_new$d_abc), improvement))

  THETAS_current <- THETAS_new
  res_current <- res_new

  if (round >= min_rounds && improvement < tol_improvement) {
    cat("Stopping: improvement below threshold.\n")
    break
  }
}

# Final THETAS
THETAS_final <- get_new_thetas(THETAS_current, res_current$d_abc, 
                                epsilon = epsilon_schedule[min(round + 1, max_rounds)])
theta_list <- c(theta_list, list(THETAS_final))


######################
## PLOT             ##
######################

pdf(sprintf(
  "results9/res_d%.3f_t%.3f_l%.3f_rho%.3f.pdf",
  d, t, l, rho
), width=8, height=8)

par(mfrow = c(2, 2))

params <- c("d", "t", "l", "rho")
true_vals <- c(dtl[1], dtl[2], dtl[3], true_rho)

for (j in 1:4) {
  boxplot(
    lapply(theta_list, function(x) x[, j]),
    main = params[j],
    xlab = "Iteration",
    ylab = "Value"
  )
  abline(h = true_vals[j], col = "red", lty = 2, lwd = 2)
}

dev.off()


######################
## SAVE             ##
######################

save(
  theta_list, d_list, dtl, true_rho,
  file = sprintf(
    "results9/res_d%.3f_t%.3f_l%.3f_rho%.3f.Rdata",
    d, t, l, rho
  )
)

